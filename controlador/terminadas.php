...
class Paginacion{
	...
	private function totalPaginas(){
		...
		$max = 'select count(*) as max from CUSTOM join ANIMES where CUSTOM.nombre_anime = ANIMES.nombre and CUSTOM.estado = "terminada" and CUSTOM.usuario="'.$this->usuario.'"';
	...
	private function obtenerSerie(){
		...
		$rango = "select ANIMES.id,ANIMES.nombre 
				...
				where CUSTOM.nombre_anime = ANIMES.nombre and CUSTOM.estado='terminada' and CUSTOM.usuario='$this->usuario'
